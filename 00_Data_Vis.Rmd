...
outTempPlot <- ggplot(
  ...
  theme_bw() +
  geom_line() +
  geom_vline(xintercept=pierson_dates$wy_doy_pierson_off[pierson_dates$waterYear==2018],color="red")+ # pierson ice-off
...
probIcePlot <- ggplot(imputed_data_trimmed_prob %>% filter(waterYear == 2018), aes(x = wy_doy, y = ice_probability)) +
  ...
  theme_bw()+
  theme(axis.title.x = element_blank()) +
  scale_x_continuous(breaks=labels_x,labels=c("30-Apr","05-May","10-May","15-May","20-May", "25-May","30-May"),limits=c(209,244))+
  annotate('rect', xmin=obs_ice_melt_windows$wy_doy_100_ice[obs_ice_melt_windows$waterYear == 2018], xmax=obs_ice_melt_windows$wy_doy_0_ice[obs_ice_melt_windows$waterYear == 2018], ymin=-Inf, ymax=Inf, alpha=0.4, fill='lightblue')

probIcePlot

probTempCQPlot <- probIcePlot / outTempPlot / outCQPlot + plot_layout(heights = c(20,20,20))
probTempCQPlot
