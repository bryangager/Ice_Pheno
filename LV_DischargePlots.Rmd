...
full_to_func <- read_xlsx("Output_Files/20240826_DischargeSlope_FullFunctional.xlsx")
# full ice to no ice
full_to_no <- read_xlsx("Output_Files/20240826_DischargeSlope_FullNo.xlsx")
# functional to no
func_to_no <- read_xlsx("Output_Files/20240826_DischargeSlope_FunctionalNo.xlsx")
## test for indexing
...
ggplot(cumulative_dat_ungroup %>%
  ...
  geom_line(aes(color = waterYear)) +
  theme_bw() +
  lims(x = c(190, 255), y = c(0, 500)) +
  labs(color = "Water Year 2014", x = "Water Year DOY", y = "Cumulative Discharge") +
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=219, color="cornflowerblue") +
  # trying to add the slope line, not working
  #geom_abline(data=full_to_func, aes(intercept=full_to_func[1,"intercept"],slope=full_to_func[1,"slope_cfs_per_day"]))
...
ggplot(cumulative_dat_ungroup %>%
  ...
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=220, color="red") 
```
...
ggplot(cumulative_dat_ungroup %>%
  ...
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=237, color="red") 
```
...
ggplot(cumulative_dat_ungroup %>%
  ...
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=242, color="red") 
```
...
ggplot(cumulative_dat_ungroup %>%
  ...
  geom_line(aes(color = waterYear)) +
  theme_bw() +
  lims(x = c(100, 240), y = c(0, 500)) +
  labs(color = "Water Year 2020", x = "Water Year DOY", y = "Cumulative Discharge") +
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=218, color="red") 

ggplot(test %>%
         filter(waterYear == 2020) %>%
         mutate(waterYear=as.factor(waterYear)), aes(x = wy_doy, y = cumulative_dis_z, group = waterYear))+
  geom_line(aes(color = waterYear)) +
  theme_bw() +
  lims(x = c(100, 240), y=c(-1,0)) +
  labs(color = "Water Year 2020", x = "Water Year DOY", y = "Cumulative Discharge") +
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=218, color="red") 
```
...
ggplot(cumulative_dat_ungroup %>%
         ...
         mutate(waterYear=as.factor(waterYear)), aes(x = wy_doy, y = cumulative_dis, group = waterYear))+
  geom_rect(aes(xmin=229,xmax=244,ymin=-Inf,ymax=Inf), fill="grey",alpha=0.1)+
  geom_line(aes(color = waterYear)) +
  theme_bw() +
  lims(x = c(200, 250), y = c(0, 500)) +
  labs(color = "Water Year 2022", x = "Water Year DOY", y = "Cumulative Discharge") +
  facet_wrap(waterYear~.)+
  geom_vline(xintercept=230, color="red")
```
