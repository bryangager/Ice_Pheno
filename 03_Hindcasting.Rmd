...
ggplot(hindcasted_ice_off_dates %>% filter(waterYear <= 2012), aes(x = waterYear, y = first_no_ice_wy_doy)) +
  theme_bw()+
  geom_point()+
  geom_point(data=obs_ice_off_dates, inherit.aes = FALSE, aes(x=waterYear, y=wy_doy_ice_off), color = "purple")+
  geom_errorbar(aes(ymin=first_no_ice_wy_doy-7, ymax=first_no_ice_wy_doy+7), width=.1,position=position_dodge(0.05))+
  # geom_errorbar(data=obs_ice_off_dates, aes(ymin=wy_doy_ice_off-7, ymax=wy_doy_ice_off+7), width=.1,position=position_dodge(0.05))+
  scale_x_continuous(
    breaks = seq(min(hindcasted_ice_off_dates$waterYear), max(hindcasted_ice_off_dates$waterYear), by = 1))+
