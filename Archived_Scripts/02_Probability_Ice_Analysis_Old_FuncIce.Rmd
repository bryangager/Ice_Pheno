...
probabilities_impute <- predict(imputed_final_model_test,
...
preds_05 <- predict(imputed_final_model_test,
  newdata = select(imputed_validation_df, -ice_presence), # remove real outcomes
  type = "response"
)
...
tab_05 <- table(imputed_validation_df$ice_presence, preds_outcome_05,
...
```{r}
# create a vector of predicted probabilities
preds_avg <- predict(imputed_final_model_test,
  newdata = select(imputed_validation_df, -ice_presence), # remove real outcomes
  type = "response"
)
...
# compare observed vs. predicted outcome
tab_avg <- table(imputed_validation_df$ice_presence, preds_outcome_avg,
  dnn = c("observed", "predicted")
)
...
```{r}
# create new probabilities for the test period:
probabilities_impute_test <- predict(imputed_final_model_test,
  newdata = select(imputed_validation_df, -ice_presence), # remove real outcomes
  type = "response")
imputed_data_trimmed_prob_test <- imputed_validation_df
imputed_data_trimmed_prob_test$ice_probability <- probabilities_impute_test


probability_impute_test_0_1 <- ifelse(probabilities_impute_test < 0.5,
  1,
  0
)
# changing the labels of the probabilities
probability_impute_factor_test <- factor(probability_impute_test_0_1, levels = c(1, 0),labels = c("ice", "no ice"))


imputed_data_trimmed_prob_test$predicted_ice <- probability_impute_factor_test
```
...
```{r}
probabilities_impute_func <- predict(imputed_final_model_test_func,
