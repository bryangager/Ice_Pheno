...
## Air Temp Variables
```{r}
# Create a cumulative freezing degree day variable (CFDD) - cumulative days below 0° using 2m mean air temp
met_hydro_winter <- met_hydro_winter %>%
  group_by(waterYear) %>%
  mutate(
    CFDD = ifelse(
...
# # Adding this DF to Input_Files
# write.csv(met_hydro_winter, "Input_Files/met_hydro_winter_ice_on.csv", row.names = FALSE)
```
# Models
## All and CFDD
```{r echo=FALSE}
...
CFDD_CLWD_model <- glm(ice_presence~CFDD+CLWD+wind_10m_mean+cumulative_dis, data = met_hydro_winter, family = binomial)
...
ggplot(met_hydro_winter_probs, aes(x = wy_doy, y = ice_probability_rollAir_cumDis, color=ice_presence)) +
  geom_point() +
  facet_wrap(~ waterYear, scales = "free_x") +
 # geom_vline(data = ice_on_dates, # this is where its maybe no bueno
...
ggplot(met_hydro_winter_probs, aes(x = wy_doy, y = ice_probability_rollAirCond, color=ice_presence)) +
  ...
  # z score conductivity only model
  zscoreCond_model <- glm(ice_presence~z_cond_uScm, data = met_hydro_winter, family = binomial)
# Table of stats n things:
tab_model(zscoreCond_model,
  show.ci = FALSE, # remove CI
  show.aic = TRUE, # display AIC
  p.style = "numeric_stars" # display p-values and stars
)
...
## Ice-On Dates
Pulling out the first day each year when ice is "ice" according to the model predictions
```{r}
# Pulling out probabilies of ice or no ice using the rolling air temperature and z-score conductivity model
probabilities_winter_met <- predict(rollingAirTempCond_model,
  newdata = select(met_hydro_winter, -ice_presence), # remove real outcomes
  type = "response")
# Adding these probabilities to the main data under a new variable name "prob_winter_daily_data"
prob_winter_daily_data <- met_hydro_winter
prob_winter_daily_data$ice_probability <- probabilities_winter_met

#Factoring the probabilities to create a predicted ice presence variable populated by "ice" or "no ice" using 0.5 as a threshold
winter_probs_met <- ifelse(probabilities_winter_met < 0.5,
  0,
  1
)
# changing the labels of the probabilities - for some reason the probabilities are backwards. This same thing happened with the ice-off predicitons.
winter_probs_factor_met <- factor(winter_probs_met, levels = c(0, 1),labels = c("ice", "no ice"))


prob_winter_daily_data$predicted_ice <- winter_probs_factor_met

# creating a dat frame with the observed and predicted ice-on dates and the difference between them
...
for(year in unique(prob_winter_daily_data$waterYear)) {
  ...
  # Find the first day when ice_presence == "ice"
  first_ice_presence <- year_data %>% 
    filter(ice_presence == "ice") %>%
    arrange(wy_doy) %>%
    slice(1)
  
  # Find the first day when predicted_ice == "ice"
...
plot_rolling7AirTemp_year(2024)
```
## Z Conductivity
```{r}
...
plot_rollingWndSpd_year <- function(year) {
  ...
  ggplot(
    met_hydro_winter %>% 
      filter(waterYear == year, wy_doy > 0),
    aes(x = wy_doy, y = wind_2day_mean, color = factor(waterYear))
  ) +
    theme_bw() +
    geom_point() +
    geom_vline(
...
```{r}
# I went through each year with the plot functions above and made these plot objects, so this won't work unless you do the same
# probTempCondPlot <- prob2014 / rollAir2014 / zcond2014 + plot_layout(heights = c(20,20,20))
# probTempCondPlot
# ggsave("Figures/Ice_On/probTempCondPlot2014.png", dpi=600, width=6, height=8, units="in")
```

# Model Outcomes
## Summary Stats
Here's some stats from the model using a rolling 7-day cumulative air temperature and the z score of daily conductivity

```{r echo=FALSE}

# compare observed vs. predicted outcome
tab_winter_met <- table(prob_winter_daily_data$ice_presence, winter_probs_met,
  dnn = c("observed", "predicted")
)
# table of model stats - R^2 and p-value - R^2 = 0.731
tab_model(rollingAirTempCond_model,
  show.ci = FALSE, # remove CI
  show.aic = TRUE, # display AIC
  p.style = "numeric_stars" # display p-values and stars
...
## Ice-On Dates
Comparing predicted (hindcasted) vs. observed ice-on dates
```{r}
...
# Plotting observed vs hindcasted ice-on water year doy
ggplot(
  ice_on_dates,
