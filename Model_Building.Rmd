...
out_dat_and_ice <- left_join(out_condTemp_dat,ice_off_binary, by = "Date") %>% mutate(Date = as.Date(Date, tz = "MST", format = "%Y-%m-%d"))
view(out_dat_and_ice)

```
# Combine all data
```{r}
cumul_out_ice_ALL <- left_join(out_dat_and_ice,cumulative_flow_df,by="Date") %>% select(-wy_doy.y) %>% rename(wy_doy = wy_doy.x)
...
model2 <- glm(ice_or_no~Temperature_C+cond_uScm, data = cumul_out_ice_ALL, family = gaussian)
summary(model2a)

model2a <- glm(ice_or_no~Temperature_C+cond_uScm+Flow, data = cumul_out_ice_ALL, family = gaussian)
summary(model2a)
