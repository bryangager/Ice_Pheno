...
out_dat_and_ice <- left_join(out_condTemp_dat,ice_off_binary, by = "Date")
view(out_dat_and_ice)
```
# Regression
```{r}
model1 <- glm(ice_or_no~Temperature_C+cond_uScm, data = out_dat_and_ice, family = binomial)
summary(model1)

model1a <- glm(ice_or_no~Temperature_C+cond_uScm, data = out_dat_and_ice, family = gaussian)
summary(model1a)

model2 <- lm(ice_or_no~Temperature_C+cond_uScm, data = out_dat_and_ice)
summary(model2)
```
