...
```{r}
# out_condTemp_long <- pivot_longer(out_condTemp_dat, -Date, names_to="hydro_variables", values_to = "temp_or_cond")
out_condTemp_long <- pivot_longer(out_condTemp_dat, -Date, names_to="hydro_variables", values_to = "temp_or_cond") %>% mutate(Date = as.Date(as.character(Date), format = "%d/%m/%y")) 
# %>% mutate(waterYear = calcWaterYear(Date))
 view(out_condTemp_long)
cond_temp_plot <- ggplot(out_condTemp_long, aes(Date,temp_or_cond, col = hydro_variables))
cond_temp_plot + geom_line()
```
