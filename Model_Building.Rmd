...
ice_presence_df <- cumul_out_ice_ALL %>% select(Date, ice_or_no, ice_presence)

# Joining weekly and imputed data with the ice presence
# weekly data frame:
flow_temp_cond_weekly_ice <- full_join(flow_temp_cond_weekly, ice_presence_df, by = "Date") %>% select(Date, wy_doy.x, waterYear.x, Flow, cumulative_dis, cond_uScm, temperature_C_raw, ice_or_no,ice_presence) %>% rename(wy_doy=wy_doy.x,waterYear=waterYear.x) %>% filter(waterYear>=2014)
View(flow_temp_cond_weekly_ice)

# imputed data frame:
flow_temp_cond_weekly_ice_impute <- full_join(flow_temp_cond_impute, ice_presence_df, by = "Date") %>% select(Date, wy_doy.x, waterYear.x, Flow, cumulative_dis, cond_uScm_impute, temperature_C_raw_impute, ice_or_no,ice_presence) %>% rename(wy_doy=wy_doy.x,waterYear=waterYear.x) %>% filter(waterYear>=2014)
View(flow_temp_cond_weekly_ice_impute)

# trimming the data frames for windows:
imputed_data_trimmed <- filter_by_year_and_doy(flow_temp_cond_weekly_ice_impute, c(183,288)) # April 1 - July 15
...
p2 <- ggplot(flow_temp_cond_impute %>% filter(waterYear.x==2012)%>% filter(wy_doy.x >188 & wy_doy.x < 288), aes(x = wy_doy.x)) +
  geom_point(aes(y = temperature_C_raw_impute, color = "Imputed Temperature (C) 2012")) +
  geom_vline(xintercept = 207)+
  labs(color = "Legend") +
  theme_minimal() +
  theme(legend.position = "top")+
...
## Imputed Modeling
```{r}
imputeModel <- glm(ice_presence~cumulative_dis+temperature_C_raw_impute, data = imputed_data_trimmed, family = binomial)
summary(imputeModel)
```
## Accuracy of Imputed
```{r}
...
plot_model(imputeModel,
  type = "pred",
  terms = c("temperature_C_raw_impute [all]"),
  ci.lvl = NA # remove confidence bands
) +
  labs(y = "Prob (no ice)")
```
...
# compare observed vs. predicted outcome
tab1 <- table(cumul_out_ice_ALL$ice_presence, preds_outcome1,
  dnn = c("observed", "predicted")
)
...
allData_trimmed <- filter_by_year_and_doy(cumul_out_ice_ALL, c(183,288)) # April 1 - July 15
# 2020 is very spotty, missing days

```
## Trimmed data models:
```{r}
trimmedDatModel <- glm(ice_presence~cumulative_dis+Temperature_C+cond_uScm+waterYear, data = allData_trimmed, family = binomial)
summary(trimmedDatModel)

reducedTrimModel <- glm(ice_presence~cumulative_dis+Temperature_C, data = allData_trimmed, family = binomial)
...
predsTrim <- predict(reducedTrimModel,
  newdata = select(allData_trimmed, -ice_presence), # remove real outcomes
  type = "response"
)
...
# compare observed vs. predicted outcome
tabTrim <- table(allData_trimmed$ice_presence, preds_outcomeTrim,
  dnn = c("observed", "predicted")
)
...
dfPred <- data.frame(observed = allData_trimmed$ice_presence, predicted = predValuesAll)
ggplot(dfPred, aes(y = predicted, x = observed))+
  geom_point()
```
## Using Predict Function to predict values on entire dataset
```{r}
testAllPredict <- cumul_out_ice_ALL
